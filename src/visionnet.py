...
class CustomVisionNetwork(TorchModelV2, nn.Module):
    ...
    def __init__(
        ...
        if not model_config.get("conv_filters"):
            model_config["conv_filters"] =[[16, [4, 4], 2],[32, [4, 4], 2],[256, [11, 11], 1],]
        ...
        filters = self.model_config.get("conv_filters")
    ...
    def forward(
        ...
        if isinstance(input_dict["obs"], list):
            print(np.shape(input_dict["obs"]))
            self._features = torch.FloatTensor(np.shape(input_dict["obs"]))
        else:
            self._features = torch.FloatTensor(input_dict["obs"])
        print("RECEIVED SHAPE:{}".format(self._features.shape))
